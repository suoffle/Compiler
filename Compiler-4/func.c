...
void checkForwardReference(){
	...
	while(id){
		...
		else if((id->kind==ID_STRUCT || id->kind==ID_ENUM) && t->field==NIL) syntax_error(32,id->name);
		id=id->prev;
	}
}
...
A_TYPE *getTypeOfStructOrEnumRefIdentifier(T_KIND k, char *s, ID_KIND kk){
	...
	if(id){
		if(id->kind==kk && id->type->kind==k){
			 if(id->type->size==0) syntax_error(60);
			 return(id->type);  
		}
...
void syntax_error(int i,char *s){
	...
	switch(i){
		...
		case 60: printf("self-reference syntax error %s",s);break;
