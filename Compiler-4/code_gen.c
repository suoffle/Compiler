...
typedef enum op {
    ...
    LDI, LDIB, SWITCH, SWVALUE, SWDEFAULT, SWLABEL, SWEND, POP, POPB
} OPCODE;
...
void gen_code_i(OPCODE op, int, int);
void gen_code_f(OPCODE op, int, float);
void gen_code_s(OPCODE op, int, char *);
void gen_code_l(OPCODE op, int, int);
...
void gen_expression(A_NODE *node)
{
    ...
    case N_EXP_POST_INC: 
        ...
        if (isPointerOrArrayType(t)) {
            gen_code_i(LITI, 0, t->element_type->size);
            gen_code_i(ADDI, 0, 0);
        } else if (isFloatType(t)) 
            gen_code_i(INCF, 0, 0);
        else gen_code_i(INCI, 0, 0);
                    ...
                    switch (t->kind) {
                        ...
                        case T_ARRAY:
                        	if(id->kind==ID_VAR)
                        		...
                        		gen_code_i(LOD,id->level,id->address);
                        	break;
                        default: gen_error(13,node->line,id->name);
                        	break;
			}
			break;
    ...
    switch (node->name) {
		...
		*/
		...
		case N_STMT_CONTINUE:
			...
			else gen_error(22, node->line);
			break;
		
		case N_STMT_BREAK:
			if(break_label) gen_code_l(JMP, 0, break_label);
			else gen_error(23, node->line);
			break;
...
void gen_declaration(A_ID *id)
{
	...
	case ID_VAR:
		if(id->init)
			if(id->level==0) gen_initializer_global(id->init, id->type, id->address);
			else gen_initializer_local(id->init, id->type, id->address);
			break;
	case ID_FUNC:
		if(id->type->expr){
