...
void sem_for_expression(A_NODE *node)
{
    ...
    switch (node->name) {
        case N_FOR_EXP:
            if (node->llink) t = sem_expression(node->rlink);
            if (node->clink) {
                t = sem_expression(node->clink);
                if (isScalarType(t))
...
int sem_A_TYPE(A_TYPE *t)
{
    ...
    case T_FUNC:
		...
		if(t->expr)  i+=sem_statement(t->expr, i, t->element_type, FALSE, FALSE, FALSE);
        t->local_var_size = i;
        break;
...
int sem_declaration(A_ID *id, int addr)
{
    ...
    switch (id->kind) {
        ...
        case ID_PARM:
	        ...
	        if (id->type == char_type) id->type = int_type;
...
{
    if ((isAnyIntegerType(t1) && isAnyIntegerType(t2)) || isFloatType(t2) || isPointerType(t2)) return (TRUE);
    else if (isFloatType(t1) && isArithmeticType(t2)) return (TRUE);
    else if (isPointerType(t1) && isAnyIntegerType(t2)) return (TRUE);
...
{
    if(t->kind==T_POINTER) return (FALSE);
...
{
    if (t && (t->kind == T_POINTER || t->kind == T_ARRAY) && t->element_type == char_type) return (TRUE);
    else
        return (FALSE);
}
...
void semantic_error(int i, int ll, char *s)
{
	...
	switch (i) {
		...
		case 95: printf("self-reference field");
